...
pipeline. Agent prompts are compiled through ``prepare_prompt``;
``RunContext`` is built at the artifact seam and is not checkpointed.
...
class RetrievalResult(BaseModel):
    ...
    Read ``items`` (and ``bodies``). Do not iterate the model: ``BaseModel``
    iteration yields field pairs, and a sequence override is not type-safe.
    """
