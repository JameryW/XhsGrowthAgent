...
def cmd_create(args: argparse.Namespace) -> int:
    ...
    if not is_monorepo(repo_root):
        ...
        if package:
            print(
                colored("Warning: --package ignored in single-repo project", Colors.YELLOW),
                file=sys.stderr,
            )
        package = None
    elif package:
        if not validate_package(package, repo_root):
            ...
            print(
                colored(f"Error: unknown package '{package}'. Available: {available}", Colors.RED),
                file=sys.stderr,
            )
            return 1
    else:
    ...
    if task_dir.exists():
        print(
            colored(f"Warning: Task directory already exists: {dir_name}", Colors.YELLOW),
            file=sys.stderr,
        )
    else:
    ...
    if args.parent:
        ...
        if not parent_json_path.is_file():
            print(
                colored(f"Warning: Parent task.json not found: {args.parent}", Colors.YELLOW),
                file=sys.stderr,
            )
        else:
            ...
            if parent_data:
                ...
                print(
                    colored(f"Linked as child of: {parent_dir.name}", Colors.GREEN), file=sys.stderr
                )
...
def cmd_archive(args: argparse.Namespace) -> int:
    ...
    if "archived_to" in result:
        ...
        print(
            colored(f"Archived: {dir_name} -> archive/{year_month}/", Colors.GREEN), file=sys.stderr
        )
...
def cmd_add_subtask(args: argparse.Namespace) -> int:
    ...
    if not parent_json_path.is_file():
        print(
            colored(f"Error: Parent task.json not found: {args.parent_dir}", Colors.RED),
            file=sys.stderr,
        )
        return 1
    ...
    if existing_parent:
        print(
            colored(f"Error: Child task already has a parent: {existing_parent}", Colors.RED),
            file=sys.stderr,
        )
        return 1
...
def cmd_remove_subtask(args: argparse.Namespace) -> int:
    ...
    print(
        colored(f"Unlinked: {child_dir.name} from {parent_dir.name}", Colors.GREEN), file=sys.stderr
    )
    return 0
