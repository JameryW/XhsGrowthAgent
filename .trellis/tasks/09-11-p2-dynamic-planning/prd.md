...
| **S5** ✅ | **契约与开闸条件**：`docs/planning.md` —— Goal / Plan 的定义、模板与"动态规划"的**开闸条件**（什么证据下才允许生成非模板图）、以及本片**明确不做**的三件事。★ 裁定：**是文档**（四条证据见本节末的 S5 小节）；另立一条本片自己的判据 —— 文档里的数字必须**可被重算**，因此它不发布任何 `file:line`（`tests/unit/scripts/test_planning_claims.py`，6 用例 / 突变 16-16）。**已交付**（`COMMIT_SHA` / [#PR_NUMBER]，见本节末的 S5 小节） | 小 |
...
- **S5 的判据是「契约本身可被检验」**：票面只要求写出文档，而本仓对文档的既有纪律是「发布 `file:line` 就要发布它的门禁」（`test_docs_anchors.py`）。`docs/planning.md` 发布的是**事实**（节点数、两个模式的计划差、计划有几个读者），所以门禁相应换成**重算**：文档里每个数字都由 `tests/unit/scripts/test_planning_claims.py` 从代码算出来并比对 —— **加一行主张而不给复核者就红**、**删一行主张而留下孤儿复核者也红**。两条**值为 0** 的主张（零生产读者、零表外读取）另配**阳性对照**，否则扫描器被掏空也不会红。
...
3. **S5 是否应该是文档**：如果 S1–S4 暴露出"确实需要非模板图"的证据，S5 就不再是文档，而是那条路径本身（P2b 的 S4 就是这种形状）。**按证据走，不按计划走。**
   **★ 已裁定（S5 交付时）：是文档。四条证据 ——** ① `.xhs/checkpoints.sqlite` 的 `checkpoints` 与 `writes` 两张表各 **0 行**、`.xhs/history/` 的 173 个文件里 172 个是 21 字节测试残留 ⇒ **零真实样本**；② 两张模板只差 **2** 个节点（trend 22 / brief 24，对称差 `{brief_analyzer, brief_gate}`，且 trend 是 brief 的**真子集**）⇒ 剪枝幅度 0–2 / 24，「动态规划更省」没有存量可省；③ S1–S4 登记的缺口**全部**是「声明不完整」而不是「声明形状错」；④ 计划今天**零生产读者**（`backend/**/*.py` 里 import `backend.graph.plan` 的 0 个）⇒ 生成出来也没人读。四条都写在 `docs/planning.md` §0，可复核。
...
没有一条边按模式分支（边表是 mode-blind 的，模式归属由 `PLAN_TEMPLATES` 声明）；`Goal` 仍未成一等输入（11 个 `workflow_mode` 读取点原样，是 S3 的正题）；`workflow.py` 未拆（S4）；没有写 `docs/planning.md`（S5 —— **已写**，见 S5 小节）。
...
- **`workflow.py` 未拆**（S4）；**`docs/planning.md` 未写**（S5 —— **已写**，见 S5 小节）。
## S3b 交付 —— Goal 成一等输入
...
- `docs/planning.md` 未写（S5 —— **已写**，见 S5 小节）。

## S5 交付 —— 契约与开闸条件

**交付物**：`docs/planning.md`（新建，184 行）+ `tests/unit/scripts/test_planning_claims.py`（新建，6 用例）+ `backend/graph/plan.py` 一处**腐烂注释**的修正。共 **3 个文件**（1 改 + 2 新）。

### ★ 裁定：S5 是文档（票面「待决」第 3 条）

票面把这一条写成判据，而不是写成计划：「如果 S1–S4 暴露出"确实需要非模板图"的证据，S5 就不再是文档，而是那条路径本身（P2b 的 S4 就是这种形状）。**按证据走，不按计划走。**」

**没有暴露。四条证据：**

| # | 证据 | 为什么它反驳「现在需要非模板图」 |
|---|---|---|
| A | **零真实样本**：`.xhs/checkpoints.sqlite` 的 `checkpoints` 与 `writes` 两张表各 **0 行**；`.xhs/history/` 的 173 个文件里 **172 个是 21 字节**的测试残留，唯一那个 115 字节的是 5 键存根（`phase=scouting`、`current_agent=trend_scout`，没有 goal、没有 niche、没有终态） | 「计划与实际不符」这件事在样本里**一次都没发生过** —— 因为没有实际 |
| B | **两张模板只差两个节点**（本片实测）：trend 22 / brief 24，对称差恰好 `{brief_analyzer, brief_gate}`，且 trend 的计划是 brief 的**真子集** | 「模板」不是两次剪枝，是**同一张图的两个入口**。剪枝幅度 **0–2 / 24** ⇒「用动态规划省节点」在本仓**没有存量可省** |
| C | **S1–S4 登记的每一处缺口都是「声明不完整」，不是「声明形状错」**：入口 path map 缺 `copywriter` 键（S2 的处置是**补全模板**）、trend 的 `phase=creating` 静默兜底、`should_brief_or_optimize` 无人调用、`execution_mode` 是**正交轴** | 全部是在模板**上**补，不是在模板**外**另起一套 |
| D | **计划今天零生产读者**：`backend/**/*.py` 里 import `backend.graph.plan` 的 **0 个** | 就算今天能生成一张非模板图，也**没有任何东西会读它** |

⇒ 结论与 S1 的定性一致：**今天缺的不是生成器，是「一次 run 的计划」这条陈述本身。** 它现在有了一半（按**模式**可陈述），另一半（按 **run** 可陈述）被写成开闸的**前置条件**。

### 做了什么

1. **两个对象的定义**（票面第一件）：`Goal`（意图 —— 模式 / 目标 / 开关 / 身份与时刻，11 字段）与 `Plan`（一个模式下可能访问的节点与 hop）。两个定义都写清了**边界**，而不是只写字段表：`Goal` 不写进 state（无 schema 变化）、`Plan` **不陈述顺序**（`steps` 按节点名排序）、`Plan` 不是执行指令（能力 / 预算 / 租约 / 闸门各有归属）。另列三个**容易撞名**的东西（`WorkflowPhase.PLANNING` 的语义是"进入 `content_strategist"`、`TaskType` 已被"模型路由键"占用、P1c 的 `ToolSpec` 是**能力**粒度）。
2. **开闸条件**（票面第二件，也是本片的正题）：闸门问的是**「一次 run 需要的计划是否与它模式的模板不同」**。计划的词汇表只有**节点 / hop / 顺序**，所以证据只能有三种形状，列出四条：**E1a**（图里有、该模式到不了 —— 今天只有 trend/brief 那一处，且它是"模式选错了"）、**E1b**（图里根本没有 —— 这才是模板吸不住的，而它同时意味着加节点加边，**会反过来裁定本片的目标本身**）、**E2**（缺一条被 `ExcludedHop` 排除的 hop —— 提它必须**指名那个 router**）、**E3**（缺一个计划根本不能表达的顺序）。另列**四条不算证据**（「任务复杂」/ 剪枝 / 想加第三个模式 / 测试里出现的新形状）与**三条前置**（计划要能按 run 陈述、要被一个能说「它错了」的检查器接住、人工闸门要能被计划表达）。
3. **明确不做的三件**（票面第三件）：不做自由的图生成 / 不动 `ExecutionMode` 轴 / 不合并 `WorkflowPhase` 的两份定义 —— 每条都写明**为什么它是一个决定，而不是一个 TODO**。
4. **登记未修汇总成 7 条**：把 S1–S4 散落在各节的登记收进一张表（每条给位置 + 不修的理由），便于下一个任务接手。

### ★ 本片自己立的一条判据：文档里的数字必须**可被重算**

票面对 S5 只要求「写出文档」。但本仓对文档已有纪律 —— `docs/execution-plane.md` 发布 `file:line`，因此配了 `tests/unit/scripts/test_docs_anchors.py`（"一个坏的锚点比没有锚点更糟，因为读者会信它"）。**不配门禁的文档就是 S1 反复抓到的那类东西：写在树上、没人复核的声明。**

所以本片给 `docs/planning.md` 配了门禁，但换了一种钉法：

| | `docs/execution-plane.md` | `docs/planning.md`（本片） |
|---|---|---|
| 发布的是 | **位置**（"这个 `create_task` 在哪一行"） | **事实**（"图有多少节点""两个模式的计划差几个""计划有几个读者"） |
| 会怎么腐烂 | 位置会**漂** | 事实不漂，事实会**被改** |
| 因此用 | `file:line` 锚点，断言 token 还在那一行 | 从代码**重算**，断言值还对 |
| 行数下限 | 必须（`>= 43` / `>= 8`）—— 行只被**检查**，从不被**枚举**，删行不可见 | **不需要** —— 复核者集合本身就是枚举，删一行会留下**孤儿复核者** |

⇒ 按位置钉事实会得到一张「行号挪了就红、而数字错了不红」的网。这就是 `docs/planning.md` **一个 `file:line` 都没有**、只引用符号的原因，理由写在它的 §5。

### 判据（票面 S5 行）如何被满足

| 票面要求 | 落在哪 |
|---|---|
| Goal / Plan 的**定义** | §1.1（含边界）+ §1.2（三个撞名物）+ §1.4（为什么模板是声明） |
| 模板与「动态规划」的**开闸条件** | §2.1 闸门问什么 / §2.2 算证据的三条四条 / §2.3 不算证据的四条 / §2.4 前置三件 / §2.5 判据的形态 |
| 本片**明确不做**的三件事 | §3，逐条给「为什么是决定而不是 TODO」 |
| （本片自加）文档的数字**可被重算** | `tests/unit/scripts/test_planning_claims.py` 的 6 条用例 |

### ★ 顺带把 S3a 的判据补全了一格

S3a 的收敛判据是 `grep -rn 'get("workflow_mode"' backend/` → 11 降到 **2**。那个形状**只覆盖 `.get(...)`，看不见下标读取**（`state["workflow_mode"]`）。本片的门禁用 AST 把**两种形状**都算：全仓读取点恰好仍是 **2**，仍**都在 `backend/state/modes.py` 内** ⇒ 判据成立，而且比原来更强。

扫描还必须**不把写入算进来**：`backend/api/routes/_wf_application.py` 有一处 `update_fields["workflow_mode"] = stored_mode(values)` —— 那是往 **DB 行**里写字段名，不是读 state（而且它读的 `stored_mode` 正是注册表的两个读者之一）。这条由**合成 fixture 的阳性对照**钉住：一个含「1 个 `.get` 读 + 1 个下标读 + 1 个下标写」的模块必须被算成 **2**，所以"连写入一起数"会得到 3、当场红。**没有这个对照，一个把写入也数进去的扫描器在真实树上仍然给出 2 / 0，两次都绿。**

### ★ 本片找到并修掉的一处腐烂

`backend/graph/plan.py` 的注释指向 `wiring.UNWIRED_ROUTERS`，而真实符号是 `ROUTERS_WITHOUT_AN_EDGE` —— 这是 S2 给它改名时留下的。**这是「腐烂是静默的」的又一个实例**：一个指向不存在符号的注释不会报错，读它的人只会以为自己找错了地方。
...
|---|---|
| `ruff check .` | 干净 |
| `ruff format --check .` | **539 files**（S4 的 538 + 1 个新测试） |
| `mypy backend --python-version 3.12` | **216** source files，no issues（本片两件新增都在 mypy 范围外：一个 `.md`、一个 `tests/`） |
| `pytest tests/unit/scripts/` | **30 passed**（该目录原 24 + 6） |
| 基线对比 / 工具运行时门禁 | drift within threshold / P1c-S5 OK |
| `pytest -q` | **3578 passed, 3 skipped**（S4 的 3572 + **6** 个新用例；新用例数**恒等于** pass 增量） |
| 突变自检 | **16/16 击杀、0 存活、0 bad-id、0 error、restore=OK**（`baseline files=6`、锚点预检 16 条各自唯一、step 0 通过） |

### 突变自检

16 条，**首轮全灭，无一条存活**。分三组，每组钉的是不同的一侧：

- **文档侧 7 条**（证明读文档的三条检查各自承重）：改一个整数（M01）/ 改一个集合值只删一个成员（M02，证明集合是**被比较**而不是被计数）/ 改一个**值为 0** 的主张（M03）/ 加一行没有复核者的主张（M04 → 只有「加行就红」那条抓到）/ 删一行主张（M05 → 只有「孤儿复核者」那条抓到）/ 把 A、B **两个**主张块的结束标记分别改名（M06、M07 —— 两条都必要，因为平衡检查必须覆盖**两个**块，只数第一对会漏掉一个）。
- **代码侧 7 条**（证明门禁是**重算**而不是复述一份冻结的拷贝）：缩 `ROUTERS_WITHOUT_AN_EDGE`（M08）/ 从 `compile_initial_state` 删一个键（M09，24→23）/ 挪 `_ROOT`（M10）/ 改 trend 的 `initial_phase`，让 `plan_registry_complaints()` 不再为空（M11）/ 给计划加上顺序（M12，`sorted(..., reverse=True)`）/ 删一条 `ExcludedHop`，**一次改动让三条主张同时移动**（M13）/ 从边表删一条边，`graph_conditional_edges` 18→17（M14）。
- **检查器侧 2 条**（证明两个**值为 0** 的主张不是空转）：把 import 扫描器掏成 `return []`（M15）/ 去掉 `Load` 过滤，让写入也算读取（M16）。**这两条是"值为 0 的主张自带不了阳性对照"的处置**：真实值本来就是 0，掏空扫描器与"确实没有"在所有真实数据上都同形，只能靠**指向有该东西的地方**（自己的目录 / 合成 fixture）才分得出来。

### S5 明确未做

- **没动任何生产逻辑**：唯一的 `.py` 改动是**一行注释**（见上）。
- **没有实现开闸后的那条路径** —— 这是裁定的结论（§0），不是遗漏。
- **没有给 `Plan` 加顺序**：§2.2 的 E3 说明它是**开闸的前置**，属于将来那件事的第一天，不属于本片。
- **没有动 `docs/execution-plane.md` 或它的锚点门禁**：本片不发布 `file:line`，两篇文档的门禁互不相干。
- 前端未动。
