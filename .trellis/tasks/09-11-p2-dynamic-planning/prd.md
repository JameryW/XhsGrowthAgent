...
| **S5** ✅ | **契约与开闸条件**：`docs/planning.md` —— Goal / Plan 的定义、模板与"动态规划"的**开闸条件**（什么证据下才允许生成非模板图）、以及本片**明确不做**的三件事。★ 裁定：**是文档**（四条证据见本节末的 S5 小节）；另立一条本片自己的判据 —— 文档里的数字必须**可被重算**，因此它不发布任何 `file:line`（`tests/unit/scripts/test_planning_claims.py`，6 用例 / 突变 16-16）。**已交付**（`6bf58d06` / [#623]，见本节末的 S5 小节） | 小 |
