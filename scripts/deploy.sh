...
cmd_start() {
    ...
    # Shared service token for trusted in-mesh calls (omp bridge / omp
    # extension → backend API). Generated once and persisted like ENCRYPTION_KEY.
    if [[ -z "${XHS_SERVICE_TOKEN:-}" ]]; then
        GENERATED_TOKEN=$(python3 -c "import secrets; print(secrets.token_urlsafe(32))")
        echo "# Auto-generated by deploy.sh — in-mesh service auth (omp → API)" >> "$PROJECT_DIR/.env"
        echo "XHS_SERVICE_TOKEN=$GENERATED_TOKEN" >> "$PROJECT_DIR/.env"
        export XHS_SERVICE_TOKEN="$GENERATED_TOKEN"
        echo ">>> 已生成 XHS_SERVICE_TOKEN 并写入 .env"
    fi
...
LLMEOF
    ...
    podman run -d \
        ...
        -e CREATOR_STATS_SYNC_INTERVAL_HOURS="${CREATOR_STATS_SYNC_INTERVAL_HOURS:-36}" \
        ...
        -e CREATOR_STATS_PRE_RUN_DELAY_MIN_SECONDS="${CREATOR_STATS_PRE_RUN_DELAY_MIN_SECONDS:-45}" \
        -e CREATOR_STATS_PRE_RUN_DELAY_MAX_SECONDS="${CREATOR_STATS_PRE_RUN_DELAY_MAX_SECONDS:-240}" \
        -e CREATOR_STATS_PERIOD_7D_CHANCE="${CREATOR_STATS_PERIOD_7D_CHANCE:-0.35}" \
