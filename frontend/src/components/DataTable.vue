...
<template>
  <div class="rounded-xl overflow-hidden relative bg-white/98 backdrop-blur-sm border border-slate-200/50">
    <!-- 表头 -->
    <div
    ...
    <!-- Empty state -->
    <div v-if="!hasData" class="p-10 text-center flex flex-col items-center gap-3">
      <div class="w-14 h-14 rounded-xl bg-slate-50 flex items-center justify-center">
        <AppIcon name="Inbox" size="xl" variant="cyan" aria-label="No data" />
      </div>
    ...
    <!-- Footer -->
    <div v-if="hasData" class="p-3 border-t border-slate-100 bg-slate-50/50 text-xs text-slate-500 text-center font-medium">
