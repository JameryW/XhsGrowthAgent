...
async function loadStep(stepId: string | null) {
  ...
  try {
    selectedStep.value = await getPublicReplayCheckpoint(publicId.value, stepId, viewMode.value === 'all' && isAuthenticated.value, { suppressToast: true })
...
async function loadReplay() {
  ...
  try {
    ...
    const [nextManifest, nextSummary] = await Promise.all([
      getPublicReplayManifest(publicId.value, technical, { suppressToast: true }),
      getPublicFinalSummary(publicId.value, { suppressToast: true }).catch(() => null),
    ])
    manifest.value = nextManifest
    finalSummary.value = nextSummary
    const first = preferredStep(nextManifest.steps)
...
watch(() => route.query.step, (value) => {
  if (typeof value === 'string' && value !== selectedStepId.value && steps.value.some(step => step.public_id === value)) void loadStep(value)
})
