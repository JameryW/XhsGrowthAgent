    ...
    <main class="relative z-sticky mx-auto max-w-6xl px-4 py-6 md:px-8 md:py-10">
      ...
      <template v-else-if="manifest">
        ...
        <section v-reveal class="glass-panel mt-7 rounded-3xl p-4 shadow-sm md:p-5" aria-labelledby="replay-steps-heading">
          ...
          <nav v-if="phaseGroups.length > 1" class="dark-explicit phase-nav-fade mt-5 overflow-x-auto border-y border-slate-200/70 py-3 dark:border-slate-800" :aria-label="t('replay.publicPhaseNavigation')"><ol class="flex min-w-max items-center gap-2"><li v-for="(phase, index) in phaseGroups" :key="phase.phase" class="phase-item"><TooltipHelper :content="phasesWithoutData.has(phase.phase) ? t('replay.phaseNoData') : ''" position="top"><button type="button" :data-phase-index="index" :tabindex="selectedPhase === phase.phase ? 0 : -1" :disabled="phasesWithoutData.has(phase.phase)" class="dark-explicit min-h-11 rounded-xl px-3 text-sm font-medium transition focus:outline-none focus-visible:ring-2 focus-visible:ring-teal-500 disabled:cursor-not-allowed disabled:opacity-40" :class="selectedPhase === phase.phase ? 'bg-gradient-to-r from-teal-600 to-cyan-500 text-white shadow-md shadow-teal-500/40 dark:from-teal-500 dark:to-cyan-400 dark:text-slate-950' : phasesWithoutData.has(phase.phase) ? 'text-slate-400 dark:text-slate-600' : 'text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800'" :aria-current="selectedPhase === phase.phase ? 'step' : undefined" :aria-disabled="phasesWithoutData.has(phase.phase)" @click="selectPhase(phase)" @keydown="handlePhaseKeydown($event, index)">{{ phaseLabel(phase.phase) }}</button></TooltipHelper></li></ol></nav>
        ...
        <!-- Keep the primary evidence visible on mobile; the whole results grid is
             taller than the viewport, so a 15% intersection threshold would
             leave it transparent until the user scrolls. -->
          ...
          <section class="glass-panel min-w-0 rounded-3xl p-5 shadow-sm md:p-7" aria-labelledby="step-detail-heading">
            <div class="replay-swap">
              ...
              <ErrorState
                ...
                role="alert"
                variant="api"
                :title="t('replay.publicDetailFailed')"
                :message="t('replay.publicDetailFailedDesc')"
                :retry-label="t('common.retry')"
                hide-dismiss
                @retry="loadStep(selectedStepId)"
              />
