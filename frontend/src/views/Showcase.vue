...
<template>
  <div class="dark-explicit showcase-v2 min-h-screen overflow-x-clip bg-slate-50 text-slate-900 dark:bg-slate-950 dark:text-slate-50">
      ...
      <div class="mx-auto flex min-h-16 max-w-7xl items-center justify-between gap-4 px-4 md:px-8">
        <button type="button" class="flex min-h-11 items-center gap-3 rounded-xl text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-teal-500" @click="router.push({ name: 'showcase' })">
          ...
          <span>
            ...
            <span class="dark-explicit hidden text-xs text-slate-500 sm:block dark:text-slate-400">{{ t('showcase.navShowcase') }}</span>
          </span>
        </button>
    ...
    <main id="main-content" class="relative z-sticky mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
      <section class="grid items-center gap-8 lg:grid-cols-[1.05fr_.95fr] lg:gap-14" aria-labelledby="showcase-title">
        <div>
          <p class="dark-explicit hero-enter inline-flex items-center gap-2 rounded-full border border-teal-200/80 bg-teal-50/90 px-3 py-1.5 text-xs font-semibold text-teal-700 shadow-sm dark:border-teal-400/20 dark:bg-teal-400/10 dark:text-teal-200" style="--enter-delay: 0ms">
          ...
          <p class="dark-explicit hero-enter mt-5 max-w-xl text-base leading-7 text-slate-600 dark:text-slate-300" style="--enter-delay: 180ms">{{ t('showcase.heroDesc') }}</p>
          <div class="hero-enter mt-6 flex flex-wrap items-center gap-3" style="--enter-delay: 270ms">
            ...
            <a href="#cases" class="dark-explicit inline-flex min-h-12 items-center rounded-xl px-4 text-sm font-medium text-slate-600 transition hover:bg-white dark:text-slate-300 dark:hover:bg-slate-900">{{ t('showcase.browseCases') }}</a>
          </div>
          <p class="dark-explicit hero-enter mt-4 flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400" style="--enter-delay: 360ms"><AppIcon name="CheckCircle" size="xs" variant="cyan" aria-hidden="true" />{{ t('showcase.heroProof') }}</p>
        </div>

        <div v-if="featuredCase" class="hero-enter relative" style="--enter-delay: 220ms">
          <div class="dark-explicit hero-ghost hero-ghost-a rounded-3xl border border-rose-200/70 bg-white/70 dark:border-rose-400/15 dark:bg-slate-900/70" aria-hidden="true" />
          <div class="dark-explicit hero-ghost hero-ghost-b rounded-3xl border border-teal-200/70 bg-white/80 dark:border-teal-400/15 dark:bg-slate-900/80" aria-hidden="true" />
          <div class="animate-float-slow relative">
            <div v-spotlight class="dark-explicit glow-border rounded-3xl border border-slate-200/80 bg-white/85 p-5 shadow-xl shadow-slate-900/5 backdrop-blur-sm dark:border-slate-800 dark:bg-slate-900/85 md:p-6" data-case-public-id="hero-featured">
              <div class="flex items-center justify-between gap-4">
                <div>
                  <p class="dark-explicit text-xs font-semibold uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">{{ t('showcase.featuredCaseBadge') }}</p>
              ...
              <div class="mt-4 flex flex-wrap items-center gap-2 text-xs">
                <span class="dark-explicit rounded-full bg-slate-100 px-2.5 py-1 font-medium text-slate-600 dark:bg-slate-800 dark:text-slate-300">{{ modeLabel(featuredCase.workflow_mode) }}</span>
                <span class="dark-explicit rounded-full px-2.5 py-1 font-medium" :class="featuredCase.status === 'completed' ? 'bg-emerald-50 dark-explicit text-emerald-700 dark:bg-emerald-400/10 dark:text-emerald-200' : featuredCase.status === 'attention' ? 'bg-amber-50 dark-explicit text-amber-700 dark:bg-amber-400/10 dark:text-amber-200' : 'bg-sky-50 dark-explicit text-sky-700 dark:bg-sky-400/10 dark:text-sky-200'">{{ statusLabel(featuredCase.status) }}</span>
                <span class="dark-explicit text-slate-500 dark:text-slate-400">{{ t('showcase.caseUpdated', { date: formatDate(featuredCase.updated_at) }) }}</span>
              </div>
              <p v-if="caseDetail(featuredCase).result_preview.topic" class="dark-explicit mt-4 rounded-xl bg-slate-50 p-3 text-sm text-slate-600 dark:bg-slate-800/70 dark:text-slate-300"><span class="dark-explicit font-medium text-slate-800 dark:text-slate-100">{{ t('showcase.detail.topic') }}：</span>{{ caseDetail(featuredCase).result_preview.topic }}</p>
      ...
      <section v-if="featuredCase" v-reveal class="mt-10" aria-labelledby="featured-heading">
        <div class="border-beam rounded-3xl shadow-xl shadow-rose-900/10">
          <div class="dark-explicit overflow-hidden rounded-3xl bg-white dark:bg-slate-900">
            <div class="grid lg:grid-cols-[.82fr_1.18fr]">
              ...
              <div class="p-6 md:p-8">
                <div v-if="detailState[featuredCase.public_id] === 'loading'" class="space-y-4" aria-busy="true"><div class="dark-explicit h-5 w-2/3 animate-pulse rounded bg-slate-200 dark:bg-slate-800" /><div class="dark-explicit h-20 animate-pulse rounded-xl bg-slate-100 dark:bg-slate-800" /></div>
                <div v-else-if="detailState[featuredCase.public_id] === 'error'" class="dark-explicit flex flex-col items-center gap-3 rounded-xl bg-rose-50 p-6 text-center dark:bg-rose-400/10" role="alert"><p class="dark-explicit text-sm font-medium text-rose-700 dark:text-rose-200">{{ t('replay.publicDetailFailed') }}</p><button type="button" class="inline-flex min-h-11 items-center rounded-xl bg-slate-900 px-4 text-sm font-semibold text-white dark:bg-white dark:text-slate-900" @click="retryFeaturedDetail">{{ t('replay.detailRetry') }}</button></div>
      ...
      <section id="cases" class="mt-16 scroll-mt-20" aria-labelledby="cases-heading">
        <div v-reveal class="relative flex flex-col justify-between gap-4 md:flex-row md:items-end">
          ...
          <div>
            <p class="dark-explicit text-xs font-semibold uppercase tracking-[0.14em] text-teal-700 dark:text-teal-300">{{ t('showcase.sectionTitle') }}</p>
            <h2 id="cases-heading" class="mt-2 text-2xl font-bold tracking-tight md:text-3xl">{{ t('showcase.evidenceTitle') }}</h2>
            <i18n-t keypath="showcase.caseCount" tag="p" class="dark-explicit mt-2 text-sm text-slate-500 dark:text-slate-400">
              <template #count><AnimatedCounter :value="resultCount" class="dark-explicit font-semibold text-teal-700 dark:text-teal-300" /></template>
            </i18n-t>
          </div>
        </div>
        ...
        <div v-reveal class="glass-panel mt-6 rounded-2xl p-3 shadow-sm md:p-4">
          <div class="flex flex-col gap-3">
            <label v-if="showSearch" class="relative block"><span class="sr-only">{{ t('showcase.searchPlaceholder') }}</span><AppIcon name="Search" size="sm" variant="cyan" class="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2" aria-hidden="true" /><input v-model="searchInput" type="search" class="dark-explicit min-h-11 w-full rounded-xl border border-slate-200 bg-white pl-10 pr-3 text-sm text-slate-800 outline-none ring-0 placeholder:text-slate-400 focus:border-rose-400 focus:ring-2 focus:ring-rose-400/20 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100" :placeholder="t('showcase.searchPlaceholder')" /></label>
            <div class="flex flex-wrap items-center gap-2">
              <button type="button" v-for="chip in statusChips" :key="chip.value" class="dark-explicit min-h-11 rounded-full px-4 text-sm font-medium transition focus:outline-none focus-visible:ring-2 focus-visible:ring-teal-500" :class="statusFilter === chip.value ? 'bg-slate-900 dark-explicit text-white shadow-md shadow-slate-900/20 ring-2 ring-teal-400/40 dark:bg-white dark:text-slate-900' : 'border dark-explicit border-slate-200 text-slate-600 hover:bg-slate-100 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800'" :aria-pressed="statusFilter === chip.value" @click="setStatusFilter(chip.value)">{{ chip.label }}</button>
              <span class="ml-auto flex items-center gap-2">
                <label class="sr-only" for="showcase-mode-filter">{{ t('showcase.filterMode') }}</label>
                <select id="showcase-mode-filter" v-model="modeFilter" class="dark-explicit min-h-11 rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-700 focus:border-teal-500 focus:ring-2 focus:ring-teal-500/20 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-200"><option value="all">{{ t('showcase.filterAll') }}</option><option value="trend">{{ t('showcase.filterTrend') }}</option><option value="brief">{{ t('showcase.filterBrief') }}</option></select>
                <label class="sr-only" for="showcase-sort">{{ t('showcase.sortRecent') }}</label>
                <select id="showcase-sort" v-model="sortKey" class="dark-explicit min-h-11 rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-700 focus:border-teal-500 focus:ring-2 focus:ring-teal-500/20 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-200"><option value="recent">{{ t('showcase.sortRecent') }}</option><option value="title">{{ t('showcase.sortTitle') }}</option></select>
              </span>
            </div>
          </div>
        </div>
        ...
        <div v-if="loading && !loaded" class="mt-5 grid gap-4 md:grid-cols-3">
          <div v-for="index in 6" :key="index" class="dark-explicit h-72 animate-pulse rounded-2xl bg-white/80 dark:bg-slate-900/80" :class="index === 1 ? 'md:col-span-2' : ''" aria-busy="true" />
        </div>
        <div v-else-if="loadError && !loaded" class="dark-explicit mt-5 rounded-2xl border border-rose-200 bg-rose-50 p-6 text-center dark:border-rose-400/20 dark:bg-rose-400/10">
          ...
          <p class="dark-explicit mt-2 text-sm text-slate-600 dark:text-slate-300">{{ t('showcase.casesLoadFailedDesc') }}</p>
        ...
        <div v-else-if="loaded && !cases.length" class="dark-explicit mt-5 rounded-2xl border border-dashed border-slate-300 bg-white/70 p-8 text-center dark:border-slate-700 dark:bg-slate-900/70">
          ...
          <p class="dark-explicit mx-auto mt-2 max-w-md text-sm leading-6 text-slate-500 dark:text-slate-400">{{ t('showcase.noPublicCasesDesc') }}</p>
        ...
        <div v-else-if="loaded && !filteredCases.length" class="dark-explicit mt-5 rounded-2xl border border-dashed border-slate-300 bg-white/70 p-8 text-center dark:border-slate-700 dark:bg-slate-900/70">
          <AppIcon name="SearchX" size="lg" variant="cyan" aria-hidden="true" />
          <h3 class="mt-3 text-base font-semibold">{{ t('showcase.noResults') }}</h3>
          <button type="button" class="dark-explicit mt-4 min-h-11 rounded-xl border border-slate-300 px-4 text-sm font-medium dark:border-slate-700" @click="clearFilters">{{ t('showcase.resetFilters') }}</button>
        </div>
        <div v-else class="mt-5 grid gap-4 md:grid-cols-3">
          <article v-for="(item, index) in filteredCases" :key="item.public_id" v-reveal="(index % 4) * 70" v-spotlight class="dark-explicit case-card glow-border group flex flex-col rounded-2xl border border-slate-200/80 bg-white/90 p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900/80" :class="caseCardSpanClass(index, filteredCases.length)" :data-case-public-id="item.public_id">
            <div class="flex items-start justify-between gap-3">
              <div class="min-w-0"><span class="dark-explicit inline-flex rounded-full bg-slate-100 px-2.5 py-1 text-xs font-medium text-slate-600 dark:bg-slate-800 dark:text-slate-300">{{ modeLabel(item.workflow_mode) }}</span><h3 class="mt-3 line-clamp-2 text-lg font-semibold leading-snug">{{ caseDetail(item).title }}</h3></div>
              <span class="dark-explicit shrink-0 rounded-full px-2.5 py-1 text-xs font-medium" :class="item.status === 'completed' ? 'bg-emerald-50 dark-explicit text-emerald-700 dark:bg-emerald-400/10 dark:text-emerald-200' : item.status === 'attention' ? 'bg-amber-50 dark-explicit text-amber-700 dark:bg-amber-400/10 dark:text-amber-200' : 'bg-sky-50 dark-explicit text-sky-700 dark:bg-sky-400/10 dark:text-sky-200'">{{ statusLabel(item.status) }}</span>
            </div>
            <p class="dark-explicit mt-3 line-clamp-3 text-sm leading-6 text-slate-600 dark:text-slate-300">{{ caseDetail(item).summary }}</p>
            <div v-if="caseDetail(item).result_preview.topic" class="dark-explicit mt-4 rounded-xl bg-slate-50 p-3 text-sm text-slate-600 dark:bg-slate-800/70 dark:text-slate-300"><span class="dark-explicit font-medium text-slate-800 dark:text-slate-100">{{ t('showcase.detail.topic') }}：</span>{{ caseDetail(item).result_preview.topic }}</div>
            <div class="mt-auto flex items-center justify-between gap-3 pt-5"><span class="dark-explicit text-xs text-slate-500 dark:text-slate-400">{{ t('showcase.caseUpdated', { date: formatDate(item.updated_at) }) }}</span><a :href="replayHref(item.public_id)" class="dark-explicit inline-flex min-h-11 items-center gap-1.5 rounded-xl px-3 text-sm font-semibold text-rose-600 transition hover:bg-rose-50 dark:text-rose-300 dark:hover:bg-rose-400/10" @click.prevent="openReplay(item.public_id)">{{ t('showcase.caseReplay') }}<AppIcon name="ArrowRight" size="xs" class="transition group-hover:translate-x-0.5" aria-hidden="true" /></a></div>
          </article>
        </div>
        <div v-if="loaded && cases.length" class="mt-5 flex flex-col items-center gap-3" aria-live="polite">
          <button v-if="hasMoreCases" type="button" class="dark-explicit min-h-11 rounded-xl border border-slate-300 bg-white px-5 text-sm font-semibold text-slate-700 transition hover:-translate-y-0.5 hover:bg-slate-50 hover:shadow-md disabled:cursor-wait disabled:opacity-60 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 dark:hover:bg-slate-800" :disabled="loadingMore" @click="loadMoreCases">
            {{ loadingMore ? t('common.loadingState') : t('showcase.loadMore') }}
          </button>
          <p v-if="hasMoreCases && loadMoreError" class="dark-explicit flex items-center gap-2 text-xs text-rose-600 dark:text-rose-300" role="alert">
          ...
          <p class="dark-explicit text-xs text-slate-500 dark:text-slate-400">
            {{ hasMoreCases
      ...
      <section v-reveal class="glass-panel relative mt-16 overflow-hidden rounded-3xl p-6 md:p-8" aria-labelledby="how-heading">
        ...
        <div class="max-w-xl"><p class="dark-explicit text-xs font-semibold uppercase tracking-[0.14em] text-teal-700 dark:text-teal-300">{{ t('showcase.howItWorks') }}</p><h2 id="how-heading" class="mt-2 text-2xl font-bold">{{ t('showcase.evidenceTitle') }}</h2></div>
        <div class="mt-8 grid gap-5 md:grid-cols-3 md:pb-10">
          <div v-for="(step, index) in ['scouting', 'creating', 'analyzing']" :key="step" v-reveal="index * 110">
            <div class="dark-explicit how-step relative rounded-2xl border border-slate-200/70 bg-white/60 p-5 transition hover:-translate-y-1 hover:shadow-lg dark:border-slate-800 dark:bg-slate-900/50" :class="index === 1 ? 'md:translate-y-5' : index === 2 ? 'md:translate-y-10' : ''">
              ...
              <p class="dark-explicit mt-2 text-sm leading-6 text-slate-500 dark:text-slate-400">{{ t(`showcase.steps.${step}`) }}</p>
            </div>
          </div>
        </div>
      </section>
    ...
    <footer class="dark-explicit relative z-sticky border-t border-slate-200/70 px-4 py-8 text-center text-sm text-slate-500 dark:border-slate-800 dark:text-slate-400">{{ t('showcase.footer') }}</footer>
  </div>
</template>
