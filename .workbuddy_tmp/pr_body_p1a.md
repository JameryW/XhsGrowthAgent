...
> **提交说明**：原 10 个提交（S1..S4-6）因本机 git 灾难丢失提交粒度，
> 以工作树快照重建为单提交 `10c972b7`——内容经逐文件哈希对账与快照一致，
> 全量门禁在本分支复验通过。下表保留为切片清单（内容清单，非提交清单）。
...
| 切片 | 内容 |
|---|---|
| S1 | 死字段删除 + 六读面收口 hydration 层 |
| S2 | 遥测外置 workflow_events |
| S3 | ArtifactStore 骨架 + 写/读接缝 + meta 摘要 |
| S4-0 | 路由读写缝补齐（~15 读面 resolve、4 写面 refify） |
| S4-1 | trend_data 外置 + trend_summary 路由 meta |
| S4-2 | ripple 向量外置 + ripple-retry 路由缝（修复 ref'd 线程 retry 永远 skipped 的真 bug） |
| S4-3 | brief_content 外置 + /status label/history/init/upload 四缝 |
| S4-4 | P0 契约等价测试（publish_id 双跑同值 / dry-run / fail-closed） |
| S4-5 | legacy 线程四读面回归（零 store 往返钉成契约） |
| S4-6 | checkpoint 体积基准（脚本 + 报告） |
| 附带 | P1b 规划文档（prd/info/consumer-map）+ Context Compiler S1 骨架（backend/context/，零行为） |

### 测试证据

- 全量 `pytest tests/unit tests/integration`：**2477 passed / 3 skipped**（基线零回归）。
- mypy strict 零错（`mypy backend --python-version 3.12`）。
- ruff check 干净。
