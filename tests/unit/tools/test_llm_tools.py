...
async def test_hashtag_researcher_llm_success():
    ...
    with (
        patch(
            "backend.tools.content.hashtag_researcher._load_prompt",
            return_value={"system": "test", "user_template": "test"},
        ),
        patch("backend.tools.content.hashtag_researcher.get_llm_service") as mock_service,
    ):
        mock_llm = MagicMock()
        mock_llm.enrich_with_llm = AsyncMock(
            return_value={
                "hashtags": [
                    {
                        "tag": "#美食",
                    ...
                    },
                    {
                        ...
                        "recommended_position": "secondary",
                    },
                ]
            }
        )
...
async def test_title_generator_llm_success():
    ...
    with (
        patch(
            "backend.tools.content.title_generator._load_prompt",
            return_value={"system": "test", "user_template": "test"},
        ),
        patch("backend.tools.content.title_generator.get_llm_service") as mock_service,
    ):
        mock_llm = MagicMock()
        mock_llm.enrich_with_llm = AsyncMock(
            return_value={
...
async def test_image_prompt_generator_llm_success():
    ...
    with (
        patch(
            "backend.tools.content.image_prompt._load_prompt",
            return_value={"system": "test", "user_template": "test"},
        ),
        patch("backend.tools.content.image_prompt.get_llm_service") as mock_service,
    ):
        mock_llm = MagicMock()
        mock_llm.enrich_with_llm = AsyncMock(
            return_value={
                "prompts": [
                    {
                        "prompt": "A modern style food scene featuring 美食, soft lighting, pastel accents",  # noqa: E501
                        "prompt_type": "cover",
                        "aspect_ratio": "3:4",
        ...
        result = await image_prompt_generator.ainvoke(
            {"topic": "美食", "style": "modern", "count": 1}
        )
...
async def test_image_prompt_generator_vintage_style():
    ...
    with patch("backend.tools.content.image_prompt.get_llm_service") as mock_service:
        ...
        result = await image_prompt_generator.ainvoke(
            {"topic": "美食", "style": "vintage", "count": 1}
        )
...
async def test_timing_optimizer_llm_success():
    ...
    with (
        patch(
            "backend.tools.scheduling.calendar._load_prompt",
            return_value={"system": "test", "user_template": "test"},
        ),
        patch("backend.tools.scheduling.calendar.get_llm_service") as mock_service,
    ):
        mock_llm = MagicMock()
        mock_llm.enrich_with_llm = AsyncMock(
            return_value={
...
async def test_timing_optimizer_niche_matching():
    ...
    with patch("backend.tools.scheduling.calendar.get_llm_service") as mock_service:
        ...
        result = await timing_optimizer.ainvoke(
            {"niche": "美食家常菜", "target_audience": "家庭主妇"}
        )

        assert "饭点" in result["niche_specific_insights"]

        # Test "穿搭分享" matches "穿搭" pattern
        result2 = await timing_optimizer.ainvoke(
            {"niche": "穿搭分享", "target_audience": "年轻女性"}
        )

        assert (
            "睡前" in result2["niche_specific_insights"]
            or "周末" in result2["reasoning"]["best_days_reason"]
        )
